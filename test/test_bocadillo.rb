...
class BocadilloTest < Test::Unit::TestCase
  ...
  def test_mutation
    ...
    reversed = @fixtures['ALBERO_FIORE'].reverse
    Bocadillo.encode(reversed)
    assert_equal reversed, @fixtures['ALBERO_FIORE'].reverse

    reversed = @fixtures['ALB_A_ER_GO_O_TO'].reverse
    Bocadillo.encode(@fixtures['ALB_A_ER_GO_O_TO'])
    assert_equal reversed, @fixtures['ALB_A_ER_GO_O_TO'].reverse
  end
